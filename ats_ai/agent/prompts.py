...
#
#
#     **PROJECTS SECTION PARSING REQUIREMENTS:**
#
#     * **Mandatory Project Extraction:**
#       * ALWAYS extract and populate the Projects array in Parsed_Resume
#       * Include ALL projects found in resume, regardless of quality
#       * Extract: Project_Name, Project_Description, Technologies_Used
#       * Do NOT skip projects during parsing phase
#
#     * **Project Validation Rules (for scoring only):**
#       * A project is VALID if it has:
#         - Meaningful project name (not "NA", "N/A", empty, or generic placeholders)
#         - AND meaningful description (not "NA", "N/A", empty, or single word)
#         - AND demonstrates actual work/implementation
#       * A project is INVALID if:
#         - Title is generic like "Project 1", "Sample Project", "NA"
#         - Description is empty, "NA", or less than 10 meaningful words
#         - No clear indication of candidate's actual work
#
#     * **Scoring Logic:**
#       * If NO valid projects found → Projects_Score = 0.0, use redistributed weights
#       * If ANY valid projects found → Score based on project quality, use standard weights
#
#     **CALCULATIONS (Perform ONLY if JD is Valid):**
#
...
#
#     CRITICAL INSTRUCTIONS:
#     1.**PRE-EVALUATION JD VALIDATION:** If the provided 'Job Description' text appears to be a short (e.g., "senior python role", "hi", "random text") :
...
#       }}
#     }}
# **CRITICAL: ALWAYS populate Projects array in Parsed_Resume even if projects are invalid for scoring purposes. The parsing and scoring phases are separate operations.**
...
def calculate_weighted_score_and_status(experience_score, skills_score, education_score, projects_score, has_valid_projects=True):
    # Dynamic weight calculation based on project validity
    if not has_valid_projects or projects_score == 0.0:
        # Redistribute weights when no valid projects
        overall_weighted_score = (experience_score * 0.4) + (skills_score * 0.5) + (education_score * 0.1)
    else:
    ...
    if overall_weighted_score >= 7.0 and match_percentage_numeric >= 70.0:
        ...
        # Determine primary reason for not qualifying
        if skills_score < 6.0:
            qualification_status = "Not Qualified - Skill Gaps"
        elif experience_score < 6.0:
            qualification_status = "Not Qualified - Insufficient Experience"
        elif projects_score < 5.0 and has_valid_projects:
            qualification_status = "Not Qualified - Lack of Project Application"
        else:
...
EVALUATION_AND_PARSING_PROMPT = """
    ...
    * **Scoring Logic:**
      * If NO valid projects found → Projects_Score = 0.0
      * If ANY valid projects found → Score based on project quality

    CRITICAL INSTRUCTIONS:
    **CRITICAL INSTRUCTIONS:**
    1.**PRE-EVALUATION JD VALIDATION:** If the provided 'Job Description' text is clearly incomplete or meaningless (e.g., only contains "hi", "test", empty string, or single random words without context):
   * Set all scores to 0.0 and status to "Not Qualified - Invalid Job Description"
   * **IMPORTANT: Job descriptions for ANY domain (technical, finance, functional, business, etc.) should be considered VALID if they contain job responsibilities and requirements, even if they don't mention programming languages or technical skills.**
   * **Only mark as invalid if the JD is genuinely unusable (empty, single words, or obvious test input).**

    **SKILL EXTRACTION FOR NON-TECHNICAL ROLES:**
    * For finance/functional roles, treat these as "skills": Oracle Financials, Oracle Fusion, AP, AR, GL, FA, CM, process design, data migration, ERP implementation, financial modules, etc.
    * For business roles, extract: domain expertise, certifications, methodologies, tools, and business processes as skills
    * Don't penalize non-technical roles for lacking programming languages - evaluate based on their domain-specific requirements
   ...
   14.**Only report a skill as missing if it's listed in the JD's Required_Skills but has no clear evidence in the resume. Do not evaluate skills not mentioned in the JD.**
    ...
    {{
      "Evaluation": {{
        ...
        "Match_Percentage": "<string, DO NOT CALCULATE - just provide individual scores>",
        "Pros": [
          "bullet point 1",
          "bullet point 2"
        ],
        ...
        "Qualification Status": "<string, DO NOT CALCULATE - just provide individual scores>",
        "Missing_Requirements": [
          "list of key requirements from JD that are not met"
        ],
        "Comments": "general suggestions or red flags",
